...
import { TvManagementDialog } from '../tv-management-dialog/tv-management-dialog';
...
@Component({
	...
	imports: [Header, Paginator, DashboardScreen, MatGridList, MatGridTile, MatIcon, Spinner, SafeHtmlPipe, TranslatePipe]
})
export class DashboardDetail implements OnInit, OnDestroy {
	...
	private isReadOnlyDashboard(): Observable<boolean> {
		return this.dashboardService.shouldDisplayedReadOnly(this.dashboardToken).pipe(
	...
	private openScreenManagementDialog(): void {
		this.matDialog.open(TvManagementDialog, {
			role: 'dialog',
			width: '700px',
			maxHeight: '80%',
			autoFocus: false,
