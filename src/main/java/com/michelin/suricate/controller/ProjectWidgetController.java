...
import static com.michelin.suricate.util.exception.constant.ErrorMessage.USER_NOT_ALLOWED_PROJECT;
...
public class ProjectWidgetController {
    ...
    @ApiResponses(
            value = {
                ...
                @ApiResponse(
                        responseCode = "404",
                        description = "Project widget not found",
                        content = {@Content(schema = @Schema(implementation = ApiErrorDto.class))})
            })
    ...
    public ResponseEntity<ProjectWidgetResponseDto> getById(
            @Parameter(name = "projectWidgetId", description = "The project widget id", required = true, example = "1")
                    @PathVariable("projectWidgetId")
                    Long projectWidgetId) {
    ...
    public ResponseEntity<List<ProjectWidgetResponseDto>> getByProject(
        ...
        List<ProjectWidget> allWidgets = project.getGrids().stream()
    ...
    public ResponseEntity<ProjectWidgetResponseDto> editByProject(
            ...
            @Parameter(name = "projectWidgetId", description = "The project widget id", required = true, example = "1")
                    ...
                    Long projectWidgetId,
            @Parameter(
                            name = "projectWidgetResponseDto",
                            description = "The project widget information to update",
                            required = true)
                    @RequestBody
                    ProjectWidgetRequestDto projectWidgetRequestDto) {
        Optional<ProjectWidget> projectWidgetOptional = projectWidgetService.getOne(projectWidgetId);
        if (projectWidgetOptional.isEmpty()) {
            throw new ObjectNotFoundException(ProjectWidget.class, projectWidgetId);
        }

        if (!projectService.isConnectedUserCanAccessToProject(
                projectWidgetOptional.get().getProjectGrid().getProject(), connectedUser)) {
            throw new ApiException(USER_NOT_ALLOWED_PROJECT, ApiErrorEnum.NOT_AUTHORIZED);
        }

        projectWidgetService.updateProjectWidget(
                projectWidgetOptional.get(),
                projectWidgetRequestDto.getCustomStyle(),
