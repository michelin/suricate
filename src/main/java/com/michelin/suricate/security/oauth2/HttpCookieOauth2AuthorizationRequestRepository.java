...
/** Authorization request repository class. Used to store the authorization request in an HTTP cookie to be stateless */
@Component
public class HttpCookieOauth2AuthorizationRequestRepository
    ...
    /**
     * Save the authorization request and the "redirect_uri" frontend parameter in cookies and attach them in the IDP
     * response. The authorization request needs to be saved somewhere because it contains a state parameter sent to the
     * IDP and expected in the IDP response to be the same, otherwise the authentication fails. It works like a CSRF
     * token. The authentication request is saved into a cookie to be stateless (stored in session by default). The
     * "redirect_uri" frontend parameter is also saved in a cookie of the IDP response to be retrieved later and used to
    ...
    /**
     * Remove the authorization request. Strange behavior here, the method needs to return the authorization request,
     * not delete it because loadAuthorizationRequest is never invoked otherwise.
     *
